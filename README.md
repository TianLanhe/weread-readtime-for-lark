# weread-readtime-for-lark

微信读书每日阅读时长同步到飞书 Base。

## Skill 介绍

读取微信读书每日阅读时长，输出日期/秒/分/时表格，也可按日期 upsert 到飞书 Base 的“阅读时长”数据表。

适用场景：读取微信读书每日阅读时长，或同步阅读时长到飞书多维表格 / Base。
...
适合希望自己控制安装路径和安装过程的用户。下面命令会把 skill 安装到 `${HOME}/.agent/skills/weread-readtime-for-lark`：

```bash
SKILL_NAME="weread-readtime-for-lark"
DOWNLOAD_URL="https://github.com/TianLanhe/weread-readtime-for-lark/archive/refs/heads/main.tar.gz"
...
安装完成后，可让支持 skills 的 agent / CLI 重新加载 skills，或开启新会话使用 `weread-readtime-for-lark`。
...
```text
https://github.com/TianLanhe/weread-readtime-for-lark/archive/refs/heads/main.tar.gz
```

可以把下面这段提示词直接交给你的 agent，让 agent 根据当前运行环境判断下载方式、安装方式以及合适的安装位置：

```text
请帮我安装这个 skill：weread-readtime-for-lark

下载地址：https://github.com/TianLanhe/weread-readtime-for-lark/archive/refs/heads/main.tar.gz
...
- “帮我读取微信读书每日阅读时长，或同步阅读时长到飞书多维表格 / Base。”
- “先只读取并打印结果，不要写入文档”
